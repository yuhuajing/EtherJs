...
const contract = await factorySC.deploy(); //deploy("WTF Token", "WTF")  如果构造函数需要参数的话，需要在deploy中提供
...
let tx = await contract.store(10000)
console.log("等待交易上链")
await tx.wait()
console.log(`store后数值: ${await contract.retrieve()}`)
