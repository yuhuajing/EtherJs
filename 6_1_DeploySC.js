import {ethers} from"ethers";

const abi = [
    "function retrieve() public view returns(uint256)",
    "function store(uint256) public",
];
const provider = new ethers.JsonRpcProvider(`https://goerli.infura.io/v3/d7b27eea18a54fb389c2562ba19f8e36`);
const walletprovite = new ethers.Wallet("797391c7bd2e156e52329ceb6471496798e0c125ef35c4c3393329bd2a64f3f5",provider)
console.log(`私钥钱包地址:${walletprovite.address}`)

async function deployOneYearLockFixture() {
const bytecode = ""
const factorySC = new ethers.ContractFactory(abi,bytecode,walletprovite);
console.log("\n等待合约部署上链")
const contractad = await factorySC.deploy(); //deploy(unlockTime, { value: lockedAmount })  如果构造函数需要参数的话，需要在deploy中提供
console.log(`合约地址: ${contractad.target}`)
//console.log(`合约地址: ${await contractad.getAddress()}`);
//console.log("部署合约的交易详情")
//console.log(await contractad.deploymentTransaction())
console.log("合约已上链")
const addr = contractad.target
return { addr };
}

async function retrieveData() {
    const { addr } = await deployOneYearLockFixture();
    const contractRW = new ethers.Contract(addr, abi, walletprovite)
    console.log("等待交易上链")
    let tx = await contractRW.store(10000)
    await tx.wait()
    console.log(`store后数值: ${await contractRW.retrieve()}`)
}

retrieveData()


